...
//   // we do not need to check for a non existing value, because that happens automatically when we scan the
// second chunk
...
//   auto scan_1 = std::make_shared<opossum::TableScan>(table_wrapper_dict_16, ColumnID{0},
// ScanType::OpGreaterThan, 200);
//   scan_1->execute();
